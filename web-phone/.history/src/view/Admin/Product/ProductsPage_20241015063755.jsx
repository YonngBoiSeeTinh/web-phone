...
const ProductsPage = () => {
  ...
  const queryClient = useQueryClient(); // Tạo đối tượng queryClient cap nhat catch khong can gopi API
      ...
      const fetchApi = async () => {
          try {
              const res = await axios.get(`http://localhost:3001/api/product/get?page=${currentPage}`);
      ...
      <div className="admin-product-container">
        <div className="admin-product-list">
          {filterListPro.map((product, index)=>(
           ...
           <Pagination totalPage={totalPage} currentPage={currentPage} setCurrentPage={setCurrentPage} style={{bottom:"-20"}} ></Pagination>       
        </div>
