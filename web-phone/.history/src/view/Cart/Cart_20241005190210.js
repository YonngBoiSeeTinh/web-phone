...
const Cart = () => {
  ...
  useEffect(() => {
    if (listCart.length > 0) {
      const selectedItems = listCart.filter((element) => element.isSelected);
      setSelectedCart(selectedItems); // Cập nhật chỉ một lần
    }
  }, [listCart]); 
  ...
  //     const total = await cart.reduce((sum, item) => sum + item.totalPrice, 0);
  //     setTotalPrice(total);
  //   };
