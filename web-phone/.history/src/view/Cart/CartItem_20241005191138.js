...
const Cart = () => {
  ...
  const [selectedCart, setSelectedCart] = useState({ totalPrice: 0, items: [] });
  ...
  // Hàm tính lại selectedCart và totalPrice khi có thay đổi
  const calculateSelectedCart = (cartList) => {
    const selectedItems = cartList.filter(item => item.isSelected);
    const total = selectedItems.reduce((sum, item) => sum + item.totalPrice, 0);

    setSelectedCart({ totalPrice: total, items: selectedItems });
  };

  // Cập nhật khi listCart thay đổi
  useEffect(() => {
    calculateSelectedCart(listCart);
  }, [listCart]);

  // Hàm xử lý khi checkbox thay đổi
  const handleSelectedCart = (cartItem, isChecked) => {
    const updatedList = listCart.map((item) =>
      item._id === cartItem._id ? { ...item, isSelected: isChecked } : item
    );
    calculateSelectedCart(updatedList); // Cập nhật selectedCart và totalPrice
  };
  ...
  return (
    <div className="cart-container">
      ...
      <div className="cart-summary">
        <p>Tạm tính ({selectedCart.items.length} sản phẩm):</p>
        <p>{selectedCart.totalPrice}đ</p>
      </div>
