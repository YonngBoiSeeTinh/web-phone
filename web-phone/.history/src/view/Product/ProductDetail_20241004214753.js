    ...
    const handleAddProduct= async()=>{
        ...
        try {
            const cartItem = {
                ...
                image: product.image,
                price: product.price,
                amount: 1, // Số lượng có thể tùy chỉnh
                userId: user.id,
                address: 'Địa chỉ người dùng', // Có thể thu thập từ thông tin người dùng
                userName: user.name,
                color: color?.color || 'Màu mặc định' // Lấy màu người dùng chọn
            };
    
            const response = await axios.post('http://localhost:3001/cart/add', cartItem);
        ...
        <div className="options">
            ...
            <div className="color-options ">
                {colors.map((colorItem, index) => (
                    <button 
                        key={index} 
                        className={`color ${colorItem.color.toLowerCase()} active`} 
