...
import { isJsonString } from './ultils/ultils';
import { axiosJWT, GetDetailUser } from './ultils/UserService';
import { refresh_token } from './ultils/UserService';
...
function App() {
  ...
  useEffect(()=>{ 
    const checkUser = async () => {
      const { storageData, decode } = handleDecode(); // Lấy token và decode
      ...
      if (decode?.id) { 
        ...
        if (decode?.exp < currentDate.getTime() / 1000) {
          try {
            // Token hết hạn, gọi hàm refresh token
            const newToken = await refresh_token(); 
            localStorage.setItem('accessToken', JSON.stringify(newToken.access_token));
            
            // Sau khi nhận được token mới, gọi hàm lấy chi tiết người dùng
            handelGetDetailUser(decode.id, newToken.access_token);
          } catch (error) {
            console.error("Lỗi khi làm mới token:", error);
          }
        } else {
          // Token còn hạn, gọi hàm lấy chi tiết người dùng
          handelGetDetailUser(decode.id, storageData);
        }
        setIsSignIn(true); // Đánh dấu người dùng đã đăng nhập
      } else {
        setIsSignIn(false); // Người dùng chưa đăng nhập
      }
    };
