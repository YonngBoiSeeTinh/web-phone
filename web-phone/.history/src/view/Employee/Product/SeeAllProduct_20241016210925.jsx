...
const SeeAllList =( {filterList})=> {
  const listFilter = filterList
   ...
   const nameList ="Danh sách sản phẩm" 
    ...
    const totalPage = Math.ceil(listFilter.length / itemsPerPage); // Tổng số trang
    
  
    // Tính toán các sản phẩm cần hiển thị cho trang hiện tại
    const startIndex = currentPage * itemsPerPage;
    const currentProducts = listFilter.slice(startIndex, startIndex + itemsPerPage);
