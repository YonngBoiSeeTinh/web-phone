...
const Cart = () => {
  ...
  const fetchApi = async () => {
    try {
      const res = await axios.get(`http://localhost:3001/api/cart/get?filter=userId&filter=${userID}`);
      console.log(user.id);
  ...
  return (
    <div className="cart-container">
      ...
      {listCart.map((cart, index) => {
        return <CartItem cart={cart} key={index} handleSelectedCart={handleSelectedCart} setTotalPrice={setTotalPrice}/>;
      })}
