  ...
  axiosJWT.interceptors.response.use(
    async function (config) {
      ...
      if (decode?.exp < currentDate.getTime() / 1000) { 
        try {
          // Gọi refresh_token() và chờ đợi phản hồi
          ...
          console.log('Hết hạn access_token:', data.access_token);
        } catch (error) {
          console.error('Lỗi khi làm mới token:', error);
          setIsSignIn(false); // Nếu có lỗi, đánh dấu là không đăng nhập
        }
      } else {
        setIsSignIn(true); // Nếu token còn hạn, xác nhận người dùng đã đăng nhập
      }
