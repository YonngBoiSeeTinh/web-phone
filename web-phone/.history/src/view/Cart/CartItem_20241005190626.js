...
const CartItem = ({ cart, handleSelectedCart, selectedCart, setSelectedCart }) => {
  ...
  const handleChange = async (e) => {
    ...
    try {
      const res = await axios.put(`http://localhost:3001/api/cart/update/${cart._id}`, isSelected );
      console.log(res.data);
  ...
  return (
    <div className="cart-item">
      ...
      <input type="checkbox" className="pickItem" onChange={handleChange} checked={cart.isSelected?true:false} />
