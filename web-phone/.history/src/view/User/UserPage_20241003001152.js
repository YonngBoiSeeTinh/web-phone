...
function UserPage ({ setIsSignIn }) {
    ...
    const [user, setUser] = useState(userFromRedux); // Copy từ Redux để lưu trữ thông tin người dùng
    ...
    const handleSaveName = async() => {    
        try {
            ...
            if (response.data.status === 'OK') {
                // Cập nhật tên trong state của user
                setUser({ ...user, name });
    ...
    const handleSaveEmail = async () => {  
        try {
            ...
            if (response.data.status === 'OK') {
                // Cập nhật email trong state của user
                setUser({ ...user, email });
    ...
    const handleSavePhone = async () => {      
        try {
            ...
            if (response.data.status === 'OK') {
                // Cập nhật phone trong state của user
                setUser({ ...user, phone });
    ...
    const handleSaveAddress = async () => {
        try {
            ...
            if (response.data.status === 'OK') {
                // Cập nhật address trong state của user
