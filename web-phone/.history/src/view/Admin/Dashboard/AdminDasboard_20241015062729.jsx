...
const fetchApi = async () => {
  try {
    const res = await axios.get(`http://localhost:3001/api/order/get?limit=20`);
...
const AdminDashboard = () => {
  ...
  const chartDataOrder = Object.entries(orderCounts).map(([month, Don_hang]) => ({
    month,
    Don_hang,
  }));
...
if (revenueMonth.previousMonth > 0) {
  ...
  compareRevenue = Math.round(compareRevenue); 
}
  ...
  return (
    <div className="dashboard">
      ...
      <div className='order_by_month'>
        ...
        <LineChart width={800} height={300} data={chartDataOrder} className='chart_order'>
          <CartesianGrid strokeDasharray="3 3" />
          <XAxis dataKey="month" />
          <YAxis />
          <Tooltip />
