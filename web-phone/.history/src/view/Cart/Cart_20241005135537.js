...
const Cart = () => {
  ...
  const handleSelectedCart = (cartItem, isChecked) => {
    setSelectedCart((prevSelected) => {
      ...
      if (isChecked) {
        return [...prevSelected, cartItem];  // Thêm sản phẩm nếu được chọn
      } else {
        return prevSelected.filter((item) => item._id !== cartItem._id);  // Loại bỏ sản phẩm nếu không được chọn
      }
    });
  };
  ...
  const handleOrder =()=>{
    ...
    console.log('Sản phẩm đã chọn:', selectedCart);
    ...
    <div className="cart-container">
      ...
      {listCart.map((cart, index) => (
        <CartItem cart={cart} key={index} handleSelectedCart={handleSelectedCart}
                   ...
                   totalPrice={totalPrice} setTotalPrice={setSelectedCart}/>
      ))}
