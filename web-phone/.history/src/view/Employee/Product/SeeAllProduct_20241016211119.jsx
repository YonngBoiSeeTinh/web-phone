...
const SeeAllList = ({ filterList = [] }) => { // Mặc định filterList là một mảng rỗng
  console.log('filterList', filterList);
  
  const [currentPage, setCurrentPage] = useState(0); // Trang hiện tại bắt đầu từ 0
  const nameList = "Danh sách sản phẩm";
  
  if (!Array.isArray(filterList)) { // Kiểm tra nếu filterList không phải là mảng
      console.error("filterList is not an array:", filterList);
      return (
